...
All Direflow Components must be placed in a folder directly inside `src`, and must all contain an `index.js` file.  
...
#### Root index.js file
There is also a __root__ `index.js` file that is places in `src/index.js`.  
