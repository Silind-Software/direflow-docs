...
In order to disable the shadow-root entirely, simply pass in an option as an argument when instantiating DireflowComponent.
- Go to *index.tsx* or *index.js* inside the *src* folder
- Pass the option `{ shadow: false }` as an argument when instantiating DireflowComponent:
```javascript
const direflowComponent = new DireflowComponent({ shadow: true });
